...
class generate(Scene):
    def construct(self):
        ...
        comma = MathTex(",").scale(1).set_color(WHITE).next_to(form_1, RIGHT, buff=0.3)
        ...
        #positions
        vector = Matrix(position).scale(0.3).set_color(WHITE).next_to(title, DOWN, buff=0.8).to_edge(LEFT, buff=0.5)
        title = Tex("Positions", color=WHITE).scale(0.4).next_to(vector, UP)
        label_y = Tex("Max. Number of Tokens = 10", color=WHITE).scale(0.3)
        label_y.rotate(-PI / 2)
        label_y.next_to(vector, LEFT, buff = 0.1)
        ...
        #arrow
        arrow = MathTex(r"\longrightarrow", color=WHITE).scale(0.5).next_to(vector, DOWN, buff=2).to_edge(LEFT).shift(UP * 0.2)
        self.play(Write(arrow))


        #Positional Encoding Map
        dic = {  "matrix":  {"values":  pe[0]                   , "scale": 0.25, "color": PURE_GREEN},
                 "title":   {"string": "Positional Encoding Map", "scale": 0.35, "color": PURE_GREEN},
        ...
        vector = Matrix(formatted_vector).scale(0.4).set_color(WHITE).next_to(arrow, RIGHT, buff=0.3)
        label_vector = Tex("Input Tokens", color=WHITE).scale(0.4).next_to(vector, UP)
        ...
        dic = { "matrix":  {"values": trim[0]                     , "scale": 0.4, "color": WHITE},
                "title":   {"string": "Positional Encoding Input" , "scale": 0.4, "color": WHITE},
