...
class generate(Scene):
    def construct(self):
        ...
        vector = Tex(f"[{formatted_vector}]").scale(0.35).set_color(WHITE).to_edge(LEFT+UP*3.5, buff=0.5)
        ...
        label = Text(f"Portuguese Tokenizer").scale(0.2).set_color(WHITE).next_to(arrow, RIGHT, buff=0.1)
        ...
        dic = { "matrix":  {"values": matrix_lines          , "scale": 0.35, "color": PURE_GREEN},
                ...
                "label_y": {"string": "Portuguese Tokenizer Size"      , "scale": 0.35, "color": PURE_GREEN, "value": vocab_size}
              }
