    ...
    def construct(self):
        ...
        def split(position,p,arg_1,arg_2,arg_3,arg_4):
            ...
            dic = { "matrix":  {"values": arg_2[0][0]                               , "scale": 0.3, "color": WHITE},
                    "title":   {"string":  f"Encoder Mask"                            , "scale": 0.3, "color": WHITE},
                    "label_x": {"string": ""                                        , "scale": 0.3, "color": WHITE, "value": ""},
                    "label_y": {"string": "Tokens"                                  , "scale": 0.3, "color": WHITE, "value": len(input_x)}
                  }
        ...
        split(position   = LEFT*0.2 + UP*6,
              p          = 1,
              arg_1      = cross_attention_scores_partial_original[0][0],
              arg_2      = [[[cross_mask[0][0][0]] * len(input_x)]],
              arg_3      = cross_attention_scores_masked[0][0],
              arg_4      = cross_attention_scores[0][0])
        

        split(position   = LEFT*0.2 + DOWN*5,
              p          = 2,
              arg_1      = cross_attention_scores_partial_original[0][1],
              arg_2      = [[[cross_mask[0][0][0]] * len(input_x)]],
              arg_3      = cross_attention_scores_masked[0][1],
              arg_4      = cross_attention_scores[0][1])
